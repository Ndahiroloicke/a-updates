...
const REGIONS = [
  ...
  { value: "ALL_AFRICA", label: "All African Countries", basePrice: 50 },
];

const PLACEMENTS = [
  { value: "RIGHT_COLUMN_TOP", label: "Right Column (Top)", basePrice: 100 },
  {
    value: "RIGHT_COLUMN_MIDDLE",
    label: "Right Column (Middle)",
    basePrice: 80,
  },
  {
    value: "RIGHT_COLUMN_BOTTOM",
...
export default function AdUploadForm() {
  ...
  const calculatePrice = useCallback(() => {
    const basePrice =
      PLACEMENTS.find((p) => p.value === formData.placement)?.basePrice || 100;
    const formatMultiplier =
      FORMATS.find((f) => f.value === formData.format)?.multiplier || 1;
    const regionMultiplier =
      REGIONS.find((r) => r.value === formData.region)?.basePrice || 10;
    const durationMultiplier =
      DURATION_TYPES.find((d) => d.value === formData.durationType)
        ?.multiplier || 1;
          ...
          {previewUrl ? (
            <div className="space-y-2">
              {formData.type === "image" ? (
                ...
                <video src={previewUrl} className="mx-auto max-h-48" controls />
              )}
