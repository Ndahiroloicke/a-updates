...
export default function AfroVideoPage() {
  ...
  const handleBackToHome = useCallback(() => {
    // Navigate to home with shallow routing to prevent full page reload
    router.push("/", { 
    ...
    <div className="h-screen bg-white flex flex-col overflow-hidden">
      <header className="fixed top-0 left-0 w-full z-50 h-14 flex items-center justify-between px-4 sm:px-6 bg-white/80 backdrop-blur-md border-b">
        <div className="flex items-center gap-3">
          <Link href="/" className="text-gray-800 hover:text-gray-600" onClick={(e) => {
            e.preventDefault();
            handleBackToHome();
          }}>
            <ArrowLeft size={22} />
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-semibold text-gray-800">Sasa</h1>
          </div>
        </div>
